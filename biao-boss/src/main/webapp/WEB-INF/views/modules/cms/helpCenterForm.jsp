...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/help_center/?category.id=${article.category.id}">文章列表</a></li>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/help_center/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章<shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/help_center/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">国际化语言:</label>
			<div class="controls">
				<form:select path="language" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('ArticleLanguage')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckfinder input="image" type="thumb" uploadPath="/cms/help_center" selectMultiple="false"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckeditor replace="content" uploadPath="/cms/help_center" />
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					$.getJSON("${ctx}/cms/help_center/findByIds",{ids:$("#articleDataRelation").val()},function(data){
					...
					$("#relationButton").click(function(){
						top.$.jBox.open("iframe:${ctx}/cms/help_center/selectList?pageSize=8", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{

...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/document/?category.id=${article.category.id}">文章列表</a></li>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/document/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章<shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/document/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckfinder input="image" type="thumb" uploadPath="/cms/document" selectMultiple="false"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckeditor replace="content" uploadPath="/cms/document" />
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					$.getJSON("${ctx}/cms/document/findByIds",{ids:$("#articleDataRelation").val()},function(data){
					...
					$("#relationButton").click(function(){
						top.$.jBox.open("iframe:${ctx}/cms/document/selectList?pageSize=8", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{

...
public class OkHttpHelper {
     ...
     * 发送get一个请给求；
     */
    public Response requestRestGet(Map<String, String> request, Map<String, String> headers, String url) throws UnsupportedEncodingException {
        Headers.Builder headersBuilder = new Headers.Builder();
        headers.forEach(headersBuilder::add);
        String join = Joiner.on("/").join(request.values());
        String requestUrl = String.format("%s/%s", url,join);
        Request request1 = new Request.Builder()
                .url(requestUrl)
                .headers(headersBuilder.build())
                .get()
    ...
    private Response getRes(String res) {
        if (StringUtils.isBlank(res)) {
            return new Response("返回的结果为空!", Constants.RESULT_FI);
        }

...
APP_CONFIG=${APP_HOME}/conf/robot.yml
if [ -n "$2" ]; then
    APP_CONFIG="$2"
fi
APP_MAIN=com.bbex.robot.Main
PIDFILE=${DIR}/PID
JAVA_OPTS="$JAVA_OPTS -server -Xms2048m -Xmx2048m -Xmn1024m -XX:ParallelGCThreads=20 -XX:+UseConcMarkSweepGC -XX:+HeapDumpOnOutOfMemoryError -XX:MaxGCPauseMillis=850 -XX:+PrintGCDetails -Xloggc:$APP_LOG/gc.log -Dfile.encoding=UTF-8"
JAVA_OPTS="$JAVA_OPTS -DlogPath=$APP_LOG -Dlog.level=info"
JAVA_OPTS="$JAVA_OPTS -Drobot.conf=${APP_CONFIG}"
#-------------------------------------------------------------------------------------------------------------
#   程序开始
#-------------------------------------------------------------------------------------------------------------
